...
import org.bouncycastle.jsl.mail.smime.SMIMEEnvelopedParser;
import org.bouncycastle.jsl.mail.smime.SMIMEUtil;
import org.bouncycastle.jsl.mail.smime.util.SharedFileInputStream;

/**
 * a simple example that reads an encrypted email using the large file model.
 * <p>
 * The key store can be created using the class in
 * org.bouncycastle.jsl.jce.examples.PKCS12Example - the program expects only one
 * key to be present.
 */
public class ReadLargeEncryptedMail
{
        ...
        throws Exception
    {
        ...
        MimeBodyPart        res = SMIMEUtil.toMimeBodyPart(recipient.getContentStream(new JceKeyTransEnvelopedRecipient((PrivateKey)ks.getKey(keyAlias, null)).setProvider(DefaultProviderName.getProviderName())));
