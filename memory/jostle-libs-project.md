...
Jostle provider facts: package `org.openssl.jostle`, provider name **`"JSL"`** (`JostleProvider.PROVIDER_NAME`), JCA/JCE-only, delegates to OpenSSL via JNI/FFI, multi-release jar (Java 8→25, built with Java 25). Built jar lives at `../openssl-jostle/jostle/build/libs/openssl-jostle-1.0-SNAPSHOT.jar`; staged into `bc-jostle-libs/libs/`. The jar exports **zero `org.bouncycastle` classes** — that gap is why `core` exists.
