...
| gap | symptom | impact |
|---|---|---|
| **No CMS3DESwrap key wrap** (MT-84) | `Cipher.getInstance` fails with `NoSuchAlgorithmException` for every name form on JSL and both FIPS modules: the OID `1.2.840.113549.1.9.16.3.6`, its `OID.`-prefixed form, `CMS3DESwrap`, `CMS3DESWrap`, `DESEDEWrap` and `DESedeWrap`. `AESWRAP` resolves on all three in the same probe, so this is the algorithm missing and not the probe misfiring. The RC2 sibling `.3.7` is absent too and is not wanted. | Gates 4 of the 16 OpenSSL ECDH vectors in `NewEnvelopedDataTest.testOpenSSLVectors` (`encSess1`, `encSess2`, `encSessA`, `encSessB`); the other 12 AES-wrap vectors pass on all three legs. Those four are gated per vector on the OID, so they self-activate when this ships - no test edit needed. Distinct from RFC 3211 key wrap (MT-93). Probed 2026-09-09 on jar `f74cadcf`, all three configurations. |
...
| **No legacy PKCS#12 PBE MAC** | `Mac.getInstance("<digest OID>")` fails for every digest OID on JSL — `1.3.14.3.2.26` (SHA-1) and `2.16.840.1.101.3.4.2.1` (SHA-256) both give "no such algorithm ... for provider JSL". The legacy PKCS#12 MAC registers its Mac under the DIGEST OID, which `JcePKCS12MacCalculatorBuilder` looks up through `createMac(algorithm.getId())`. Enumerating the provider's services shows its whole PKCS#12 surface is `KeyStore.PKCS12`, `PKCS12-AES256-AES128`, `PKCS12-3DES-3DES` and `PKCS12-PBMAC1` — four KeyStore entries and no Mac. Not a digest-choice problem: SHA-1 and SHA-256 fail alike. | Gates the two PBE cells of `PKCS12UtilTest` (`testConvertToDefiniteLength_PBE_RoundTrips`, `_Idempotent`). **PBMAC1 is unaffected** — `testConvertToDefiniteLength_PBMAC1_RoundTrips` runs and covers the same `convertToDefiniteLength` code through the modern MAC, so the method is not left untested. Probed 2026-09-14 on jar `dcd7d727`. |
