 ...
 * <code>Security.getProvider("JSLJSSE").getInfo()</code> says which of the two jars is installed -
 * they otherwise present the same provider name, version and services, and only the jar decides
 * whether the JVM is able to disclose its own TLS secrets. See <code>org.bouncycastle.jsl.tls.KeyLog</code>
