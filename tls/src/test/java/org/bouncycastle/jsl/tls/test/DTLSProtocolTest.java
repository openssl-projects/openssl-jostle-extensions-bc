...
{
    /**
     * These handshakes authenticate with RSA certificates over DTLS 1.2, which signs a
     * caller-supplied digest through NoneWithRSA. A FIPS module registers that and refuses it at
     * initSign, so probe an actual signature rather than the service. A TestCase subclass cannot
     * skip via Assume, so the class returns early instead.
     */
