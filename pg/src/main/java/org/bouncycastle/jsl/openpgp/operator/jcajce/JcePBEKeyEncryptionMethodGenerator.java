    ...
    extends PBEKeyEncryptionMethodGenerator
{
    ...
    {
        return JceAEADUtil.generateHKDFBytes(helper, ikm, null, info, SymmetricKeyUtils.getKeyLengthInOctets(kekAlgorithm));
    }
        ...
        throws PGPException
    {
        ...
        try
        {
            // Apply the AAD via updateAAD (setUpAeadCipher), not the associated-data field of
            // AEADParameterSpec: the JSL/OpenSSL AEAD ciphers don't read AAD out of that BC spec,
            // which would leave the tag computed without the AAD and break the decrypt's auth.
            JceAEADCipherUtil.setUpAeadCipher(aeadCipher, new SecretKeySpec(key, algorithm),
                Cipher.ENCRYPT_MODE, iv, 128, info);
            return aeadCipher.doFinal(sessionKey, 0, sessionKey.length);
        }
