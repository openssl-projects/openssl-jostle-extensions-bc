            ...
            return new PGPSecretKeyDecryptorWithAAD(passPhrase, calculatorProvider, new JcePGPS2KCalculator(helper))
            {
                    ...
                    throws PGPException
                {
                    try
                    {
                        ...
                        // AAD via updateAAD (setUpAeadCipher), not AEADParameterSpec's associated-data
                        // field, which the JSL/OpenSSL AEAD ciphers don't read.
                        JceAEADCipherUtil.setUpAeadCipher(c, JcaJcePGPUtil.makeSymmetricKey(encAlgorithm, key),
                            Cipher.DECRYPT_MODE, iv, 128, aad);
