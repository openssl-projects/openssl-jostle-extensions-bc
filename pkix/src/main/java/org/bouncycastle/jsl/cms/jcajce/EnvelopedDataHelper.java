...
public class EnvelopedDataHelper
{
        ...
        throws CMSException
    {
        if (algId.getAlgorithm().equals(CMSObjectIdentifiers.id_alg_cek_hkdf_sha256))
        {
            ...
            // RFC 9629 id-alg-cek-hkdf-sha256: derive the CEK with HKDF-SHA256 through the
            // provider's native HKDF SecretKeyFactory (info == the DER enc-algorithm identifier).
            try
            {
                SecretKeyFactory hkdf = createSecretKeyFactory("HKDF-SHA256");
                keyData = hkdf.generateSecret(new HKDFParameterSpec(
                    keyData, hkdfSalt, encAlgId.getEncoded(ASN1Encoding.DER), keyData.length)).getEncoded();
            }
            catch (IOException e)
            {
