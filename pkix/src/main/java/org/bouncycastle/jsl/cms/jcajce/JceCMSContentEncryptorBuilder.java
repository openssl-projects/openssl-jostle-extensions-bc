...
import org.bouncycastle.jsl.jcajce.io.CipherOutputStream;
import org.bouncycastle.jsl.operator.DefaultSecretKeySizeProvider;
import org.bouncycastle.jsl.operator.GenericKey;
import org.bouncycastle.jsl.operator.MacCaptureStream;
import org.bouncycastle.jsl.operator.OutputAEADEncryptor;
import org.bouncycastle.jsl.operator.OutputEncryptor;
import org.bouncycastle.jsl.operator.SecretKeySizeProvider;
...
public class JceCMSContentEncryptorBuilder
{
    ...
    private class CMSOutEncryptor
    {
        ...
        private void applyKdf(ASN1ObjectIdentifier kdfAlgorithm, AlgorithmParameters params, SecureRandom random)
            throws CMSException
        {
            // HKDF-SHA256 derived through the provider's native HKDF SecretKeyFactory
            // (RFC 9629 CEK derivation; info == the DER enc-algorithm identifier).
            byte[] encKeyEncoded = encKey.getEncoded();
            try
            {
                SecretKeyFactory hkdf = helper.createSecretKeyFactory("HKDF-SHA256");
                encKeyEncoded = hkdf.generateSecret(new HKDFParameterSpec(
                    encKeyEncoded, hkdfSalt, algorithmIdentifier.getEncoded(ASN1Encoding.DER), encKeyEncoded.length)).getEncoded();
            }
            catch (IOException e)
            {
            ...
            {
                throw new CMSException("unable to derive content-encryption key: " + e.getMessage(), e);
            }
        ...
        implements OutputAEADEncryptor
    {
        ...
        public OutputStream getAADStream()
        {
            ...
            return null; // TODO: okay this is awful, we could use a BC AEAD parameter spec for earlier JDKs.
        }
