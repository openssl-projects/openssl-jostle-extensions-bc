    ...
    implements Destroyable
{
    ...
    private static BigInteger validate(BigInteger modulus, boolean isInternal)
    {
        ...
        if (!isInternal)
        {
            ...
            if (Properties.isOverrideSet("org.bouncycastle.jsl.rsa.allow_unsafe_mod"))
            {
                return modulus;
            }

            int maxBitLength = Properties.asInteger("org.bouncycastle.jsl.rsa.max_size", 16384);
            ...
            int iterations = Properties.asInteger("org.bouncycastle.jsl.rsa.max_mr_tests", defaultIterations);
            if (iterations > 0)
            {
